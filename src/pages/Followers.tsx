...
// apis
import { fetchFollowers, fetchUser } from "@src/libs/fetcher";
...
const Followers = () => {
    ...
    const { id } = useParams();
    const { auth } = useApp();
    const { data, isLoading, isError, error } = useQuery<unknown, Error, UserProps[], QueryKey>("followers", async () => fetchFollowers(id as string));

    const { data: user } = useQuery<unknown, Error, UserProps, QueryKey>("user", () => fetchUser(id as string));
    const isMe = auth?.id == id;
    const title = isMe ? "Your Followers" : `${user?.username}'s Followers`;
